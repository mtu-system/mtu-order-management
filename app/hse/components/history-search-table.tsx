...
type ResultFilter = 'all' | 'passed' | 'failed' | 'full_vm'

const resultOptions: { value: ResultFilter; label: string }[] = [
  { value: 'all', label: 'Semua Hasil' },
  { value: 'passed', label: 'Semua Passed' },
  { value: 'failed', label: 'Ada Unit Failed' },
  { value: 'full_vm', label: 'Full VM' },
]
...
export default function HistorySearchTable({ orders }: HistorySearchTableProps) {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((order) => {
      ...
      if (resultFilter === 'passed' && order.resultLabel !== 'Semua Passed') {
        return false
      }
      if (resultFilter === 'failed' && !order.resultLabel.includes('Failed')) {
        return false
      }
      if (
        resultFilter === 'full_vm' &&
        order.resultLabel !== 'Full VM — Tidak Perlu HSE'
      ) {
        return false
      }
  ...
  }, [orders, search, showPk, showRft, resultFilter, dateFrom, dateTo])
  ...
  function handleExport() {
    const rows = filteredOrders.map((order) => ({
      ...
      'Kendaraan Internal': order.vehicleSummary || '-',
      'Unit VM': order.vmCount,
      Trip: order.trip || '-',
      Hasil: order.resultLabel,
      Tanggal: order.dateLabel,
    }))
    const worksheet = XLSX.utils.json_to_sheet(rows)
    const workbook = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(workbook, worksheet, 'HSE History')
    const today = new Date().toISOString().slice(0, 10)
    XLSX.writeFile(workbook, `hse-history-${today}.xlsx`)
  }
            ...
            {!filteredOrders.length ? (
              ...
              filteredOrders.map((order) => (
                <tr key={order.id} className="transition-colors hover:bg-gray-50/60">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-bold ${order.resultClass}`}>
                      {order.resultLabel}
                    </span>
                  </td>
