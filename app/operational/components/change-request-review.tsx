...
export default function ChangeRequestReview({
  ...
  const [replacementUnits, setReplacementUnits] =
    useState<NewUnit[]>([])
   ...
   * ======================================================
   *
   * Kalau trucks.length === 0 berarti Operational belum
   * mengisi detail kendaraan sama sekali.
   *
   * Kalau sudah ada truck -> flow lama tetap dipakai.
   */
   ...
   * - change_vehicle
   *
   * Tidak membuat order_trucks.
   * Hanya mengubah order + order_requirements melalui RPC.
   */
  ...
  async function handleApproveWithoutTrucks() {
    ...
    try {
      ...
      if (error) {
        console.error(
          'APPLY CHANGE WITHOUT TRUCK ERROR:',
          error
        )
      ...
      if (orderAfterError) {
        console.error(
          'GET ORDER AFTER AUTO CHANGE ERROR:',
          orderAfterError
        )
      }
      ...
      const { error: activityError } =
        await supabase
          ...
          .insert({
            ...
            user_id: user.id,
            action:
              'AUTO_APPROVE_CHANGE_REQUEST_NO_TRUCKS',
      ...
      if (activityError) {
        console.error(
          'AUTO CHANGE ACTIVITY LOG ERROR:',
          activityError
        )
      }
      ...
      console.error(
        'AUTO CHANGE WITHOUT TRUCK ERROR:',
        error
      )

      alert(
        'Terjadi kesalahan saat menerapkan perubahan tanpa detail truck.'
      )
    } finally {
      setLoading(false)
    }
  ...
  async function handleApproveReduce() {
    ...
    try {
      ...
      const {
        data: trucksBefore,
        error: trucksBeforeError,
      } = await supabase
        .from('order_trucks')
        .select(
          'id, status, vehicle_type, plate_number'
        )
      ...
      const { error } = await supabase.rpc(
        'approve_reduce_unit_request',
        {
          p_request_id: request.id,
          p_selected_truck_ids:
            selectedTruckIds,
        }
      )
  ...
  async function handleApproveAdd() {
    ...
    try {
      ...
      const { error } = await supabase.rpc(
        'approve_add_unit_request',
        {
          p_request_id: request.id,
          p_units: newUnits.map((unit) => ({
            vehicle_type:
              unit.vehicle_type.trim(),
            ...
            no_buntut:
              unit.no_buntut.trim() || null,

            plate_number:
              unit.plate_number.trim(),

            driver_name:
              unit.driver_name.trim(),

            driver_phone:
              unit.driver_phone.trim(),
          })),
        }
      )
      ...
      if (!trucksAfterError) {
        const newlyAddedTrucks =
          (trucksAfter || []).filter(
            (truck) =>
              !oldTruckIds.has(truck.id)
          )
  ...
  async function handleApproveChangeVehicle() {
    ...
    if (!request.requested_vehicle_type) {
      alert(
        'Jenis kendaraan baru belum ditentukan.'
      )
      return
    }
    ...
    try {
      ...
      const {
        ...
        .from('order_trucks')
        .select(
          'id, status, vehicle_type, plate_number, driver_name'
        )
        .eq('order_id', orderId)
      ...
      await logOrderHistory({
        ...
        action: 'change_vehicle',
        fieldName: 'vehicle_type',
        oldValue: selectedTrucks
          .map(
            (truck) =>
              truck.vehicle_type
          )
          .join(', '),
        newValue: replacementUnits
          .map(
            (unit) =>
              unit.vehicle_type
          )
      ...
      if (activityError) {
        console.error(
          'CHANGE VEHICLE ACTIVITY LOG ERROR:',
          activityError
        )
      }
      ...
      console.error(
        'CHANGE VEHICLE ERROR:',
        error
      )

      alert(
        'Terjadi kesalahan saat mengganti jenis unit.'
      )
    } finally {
      setLoading(false)
    }
    ...
    try {
      ...
      const {
        ...
        .not(
          'status',
          'in',
          '(cancelled,departed,finished)'
        )

      if (trucksError) {
        console.error(
          'GET ACTIVE TRUCKS ERROR:',
          trucksError
        )
      ...
      /*
       * Kalau ada truck, cancel truck.
       * Kalau tidak ada truck, tidak masalah.
       */

      if (trucksBefore?.length) {
        const { error: truckError } =
          await supabase
            .from('order_trucks')
            .update({
              status: 'cancelled',
  ...
  async function handleApproveValueChange() {
    ...
    if (
      ...
      alert(
        'Jenis perubahan ini tidak didukung.'
      )
      return
    }

    if (
      !request.requested_value?.trim()
    ) {
      alert(
        'Nilai perubahan belum diisi.'
      )
      return
    }
    ...
    try {
      ...
      const fieldName =
        fieldMap[request.change_type]

      if (!fieldName) {
        alert(
          'Field perubahan tidak ditemukan.'
        )
        return
      }
      ...
      if (activityError) {
        console.error(
          'VALUE CHANGE ACTIVITY LOG ERROR:',
          activityError
        )
      }
      ...
      console.error(
        'APPROVE VALUE CHANGE ERROR:',
        error
      )

      alert(
        'Terjadi kesalahan saat menerapkan perubahan order.'
      )
    } finally {
      setLoading(false)
    }
  ...
  async function handleReject() {
    ...
    const confirmed = window.confirm(
      'Tolak permintaan perubahan ini?'
    )
    ...
    try {
      ...
      alert(
        'Terjadi kesalahan saat menolak request.'
      )
    } finally {
      setLoading(false)
    }
    ...
    <div className="mb-6 overflow-hidden rounded-2xl border border-amber-200 bg-white shadow-sm">
      ...
      <div className="border-b border-amber-100 bg-amber-50/60 px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full bg-white px-2.5 py-1 text-xs font-medium text-amber-700 ring-1 ring-inset ring-amber-200">
        ...
        <p className="mt-1 text-sm text-amber-800/80">
          Marketing mengajukan perubahan pada
          order ini.
        </p>
      </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-blue-700">
                    Operational belum mengisi
                    detail kendaraan. Perubahan
                    tetap bisa disetujui sekarang.
                    Sistem hanya akan mengubah
                    jumlah dan kebutuhan unit.
                  </p>

                  <p className="mt-2 text-xs font-medium text-blue-800">
                    Setelah itu Operational tinggal
                    mengisi detail kendaraan sesuai
                    jumlah terbaru.
                  </p>
                </div>
          ...
          <div>
            ...
            {hasTruckDetails ? (
              <>
                <div className="mb-3">
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    Operational menentukan unit
                    mana yang dikurangi berdasarkan
                    antrean dan kondisi unit.
                  </p>
                </div>
              ...
              <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800">
                Tidak ada detail truck yang perlu
                dipilih. Pengurangan akan langsung
                diterapkan ke jumlah kebutuhan order.
              </div>
            )}
            ...
            <div className="mt-5 flex justify-end gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-40"
              >
                {loading ? (
        ...
        {request.change_type ===
          ...
          <div className="mt-5">
            ...
            <div className="mb-4">
              ...
              <p className="mt-1 text-xs text-gray-500">
                Permintaan mengganti{' '}
                {requestedQuantity} unit menjadi{' '}
                <strong>
                  {request.requested_vehicle_type ||
                    '-'}
                </strong>
            ...
            {hasTruckDetails ? (
              <>
                ...
                <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4">
                  ...
                  <p className="mt-1 text-xs text-amber-700">
                    Unit lama yang dipilih akan
                    berstatus Cancelled. Unit
              ...
              <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800">
                Tidak ada detail truck yang perlu
                dipilih. Sistem hanya akan mengubah
                kebutuhan kendaraan dari{' '}
                <strong>
                  {request.requested_vehicle_type ||
                    '-'}
                </strong>
            ...
            <div className="mt-5 flex justify-end gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-40"
              >
                {loading ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
          ...
          <div className="mt-5">
            ...
            {hasTruckDetails ? (
              ...
              <div className="rounded-xl border border-blue-200 bg-blue-50 p-4">
                ...
                <p className="mt-1 text-sm text-blue-700">
                  Detail kendaraan belum ada,
                  jadi tidak perlu mengisi plat,
                  driver, atau data kendaraan.
                  Sistem hanya menambah jumlah
                  kebutuhan unit.
                </p>
        ...
        {[
          ...
          <div className="mt-5">
            ...
            <div className="rounded-xl border border-blue-200 bg-blue-50 p-5">
              ...
              <p className="mt-1 text-sm text-blue-700">
                Nilai baru akan diterapkan
                langsung ke order setelah
                disetujui.
              </p>
          ...
          <div className="mt-5">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-red-700">
                    Marketing mengajukan
                    pembatalan order ini.
                    Jika disetujui, order akan
                    berstatus{' '}
                    <strong>
                      Cancelled
                    </strong>{' '}
                    dan seluruh unit yang
            ...
            <div className="mt-5 flex justify-end gap-3">
              ...
              <button
                ...
                disabled={loading}
                className="inline-flex items-center gap-1.5 rounded-lg bg-red-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-red-700 disabled:opacity-50"
              >
                {loading ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
