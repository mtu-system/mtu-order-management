...
export default function UnitAllocationForm({
  ...
  function isValidAllocation(allocation: Allocation) {
    const required = getRequirementQuantity(allocation.vehicle_type)
  ...
  async function handleSubmit(
    ...
    try {
      ...
      if (existingLogError) {
        console.error('CHECK EXISTING ALLOCATION ERROR:', existingLogError)
      }

      const { data: existingTrucks, error: existingTruckError } =
        await supabase
          .from('order_trucks')
      ...
      if (existingTruckError) {
        console.error('CHECK EXISTING TRUCK ERROR:', existingTruckError)
      }
      ...
      const vmRows = allocations.flatMap((allocation) =>
        Array.from({ length: allocation.vendor }, () => ({
          order_id: orderId,
          source: 'vendor',
      ...
      const totalInternal = allocations.reduce(
        (total, allocation) => total + allocation.internal,
        0
      )

      const totalVm = allocations.reduce(
        (total, allocation) => total + allocation.vendor,
        0
      )

      const totalUnavailable = allocations.reduce(
        (total, allocation) => total + allocation.unavailable,
        0
      )
  ...
  return (
    ...
    >
      <div className="mb-6 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500">
            Tentukan berapa unit yang berasal dari Internal, Vendor/VM, dan
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            {allocations.map((allocation, index) => {
              const required = getRequirementQuantity(
      ...
      <div className="mt-4 rounded-xl bg-gray-50 p-4 text-sm text-gray-600">
        <div className="font-semibold text-gray-900">Contoh:</div>
        <div className="mt-1">
          Kebutuhan 15 Trailer → 10 Internal + 3 Vendor/VM + 2 Tidak
          Tersedia.
        </div>
        <div className="mt-2 text-xs text-gray-500">
          Hanya unit Internal yang akan dikirim ke HSE. Unit Vendor/VM
          otomatis dicatat sebagai VM dan tidak masuk pemeriksaan HSE.
        </div>
      </div>
